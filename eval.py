...
def evaluate_model(path_to_model_dir: Path, eval_config: utils.config.EvalConfig):
    ...
    eval_model = model.build.build_extended_ae_model(config, preset_idx_helper)
    eval_model.load_state_dict(checkpoint['ae_model_state_dict'])
    eval_model = eval_model.to(device).eval()
    ae_model, reg_model = eval_model.ae_model, eval_model.reg_model
    ...
    dynamic_controls_num_mae_crit = \
        model.loss.QuantizedNumericalParamsLoss(dexed_dataset.preset_indexes_helper,
                                                numerical_loss=nn.L1Loss(reduction='mean'),
    ...
    for i, sample in tqdm(enumerate(dataloader), total=len(dataloader)):
        x_wav, x_in, v_in, preset_UID = sample[0].to(device), sample[1].to(device), sample[2].to(device), sample[3]
        
        with torch.no_grad():
            ae_out = ae_model(x_in, preset_UID)
